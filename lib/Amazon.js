        ...
        for (let key in scrapingResult) {
            try {
                ...
                try {
                    ...
                    } } catch { console.log('Amazon.js > titleThumbnail error'); }

                    try {
                    if (priceSearch) {
                        scrapingResult[key].price.current_price = this.geo.price_format($(priceSearch.children[0]).text());
                    } } catch { console.log('Amazon.js > priceSearch error'); }

                    try {
                    if (amazonChoice) {
                        scrapingResult[key].amazonChoice = true;
                    } } catch { console.log('Amazon.js > amazonChoice error'); }

                    try {
                    if (bestSeller) {
                        scrapingResult[key].bestSeller = true;
                    } } catch { console.log('Amazon.js > bestSeller error'); }

                    try {
                    if (amazonPrime) {
                        scrapingResult[key].amazonPrime = true;
                    } 
                } catch { console.log('Amazon.js > amazonPrime error'); }
                ...
                } catch { console.log('Amazon.js > discountSearch error'); };
                ...
                } catch { console.log('Amazon.js > ratingSearch error'); }
