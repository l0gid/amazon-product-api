...
class AmazonScraper {
    ...
    httpRequest({ uri, method, qs, json, body, form }) {
        ...
        const currencyCookie = `i18n-prefs=${this.geo.currency}`; // Ensures the currency matches the domain
        ...
        return new Promise(async (resolve, reject) => {
            const options = {
                ...
                headers: {
                    ...
                    cookie: this.cookie + '; '  + currencyCookie,
    ...
    async startScraper() {
        if (this.scrapeType === 'products') { // Passed in number / products per page to determine how many pages to scrape
            this.asyncPage = Math.ceil(this.number / 15);
            if (!this.keyword) {
                throw new Error('Keyword is missing');
            }
            if (this.number > CONST.limit.product) { // This is an internal limit, nothing to do with how many results there are
        ...
        console.log('collector content size after mainLoop: ' + this.collector.length);

        this.sortAndFilterResult();

        console.log('collector content size after sortAndFilter: ' + this.collector.length);
        ...
        if (this.fileType && this.cli) {
            console.log(`Result was saved to: ${path.resolve(this.path)}`);
        }
        return {
        ...
        return new Promise((resolve, reject) => {
            forEachLimit(
                ...
                (item, cb) => {
                    ...
                    const run = () => {
                        Promise.resolve()
                            .then(() => this.buildRequest(this.bulk ? item : this.searchPage))
                            .then(body => {

                                // New: raw-as html mode
                                if (this.fileType == 'html') {
                                    this.collector.push(body); // Push the body to the collector!
                                    throw new Error("Done"); // skip normal processing
                                }
                            ...
                            .catch(err => {
                                if (err && err.message === 'Done') {
                                    cb(err); // Early exit, do not retry
                                } else {
                                    attempt++;
                                    if (attempt < maxRetries) {
                                        delay(750 * attempt).then(run);
                                    } else {
                                        cb(err);
                                    }
                                }
    ...
    async saveResultToFile(savePath) {
        if (this.collector.length) {
            ...
            case 'html':
                await fromCallback((cb) => writeFile(`${baseFilePath}.html`, this.collector.join(""), cb));
                break;
            case 'json':
                await fromCallback((cb) => writeFile(`${baseFilePath}.json`, JSON.stringify(this.collector, null, 2), cb));
                break;
            case 'csv':
        ...
        } else { console.log('No data reached saveResultToFile, check this.sortAndFilterResult'); }
    }
    ...
    grabAsinDetails(body) {
        ...
        output.price.current_price = $(`span.a-price.aok-align-center .a-offscreen`)[0] 
        ? this.geo.price_format($(`span.a-price.aok-align-center .a-offscreen`).first().text()) 
        : 0;
        if (!output.current_price) {
            try {
                output.price.current_price = this.geo.price_format($($(`span.a-price.apexPriceToPay`)[0].children[0]).text());
                //console.log("Trying to find price at span.a-price.apexPriceToPay");
            } catch {
                try {
        ...
        } //else { console.log("price found at span.a-price.aok-align-center .a-offscreen"); }
        ...
        // Code to try get "This item" details from Compare with similar items table

        let comparisonTable = $(`table[class*="_product-comparison-desktop_desktopFaceoutStyle_comparisonTable__"]`).first();
        if (comparisonTable.length) { // We found the table!
                let skipRows = 4;
                let dataMap = {};
                comparisonTable.find('tr').slice(skipRows).each((i, row) => {
                    let cells = $(row).find('td');
                    if (cells.length >= 2) { // ensure at least 2 columns exist
                ...
                output.compareThisItem = dataMap;
        }


        // Code to try get "Technical details" from product information

        let prodDetails = $('#prodDetails').find('.a-expander-content').first();
        if(prodDetails.length) { // exists!

            // Initialize a map (object) to hold key-value pairs
            let detailsMap = {};

            // Find all table rows (tr) that are descendants of prodDetails
            prodDetails.find('tr').each((i, row) => {
                let th = $(row).find('th').first();
                let td = $(row).find('td').first();

                // Only add if both th and td exist
                if (th.length && td.length) {
                    let key = th.html().trim();     // use .html() to preserve innerHTML
                    let value = td.html().trim().replace(/\u200E/g, '');   // use .html() for innerHTML, remove any left right encoding
                    detailsMap[key] = value;
                }
            });

            // Log the resulting map
            output.techDetails = detailsMap;
    ...
    grabProduct(body, p) {
        ...
        for (let key in scrapingResult) {
            try {
                ...
                const ratingSearch = $(`div[data-asin=${key}] [data-cy="reviews-block"]`).first();

                const thumbnailSearch = $(`div[data-asin=${key}] [data-image-source-density="1"]`)[0];
                const titleSearch = $(`div[data-asin=${key}] [data-cy="title-recipe"] h2`)[0];
                ...
                // Check if the listing is showing as having a coupon on the search page
                const couponSearch = $(`div[data-asin=${key}] span[data-component-type="s-coupon-component"]`);
                

                    try { 
                        scrapingResult[key].hasCoupon = couponSearch.length > 0;
                    } catch (e) { console.log('Amazon.js > Coupon error'), e;}


                    try {
                    if (thumbnailSearch) {
                        //scrapingResult[key].title = titleThumbnailSearch.attribs.alt; // Moved this so be separate with a different accessor, this title was concatenated...
                        scrapingResult[key].thumbnail = thumbnailSearch.attribs.src;
                    } } catch (e) { console.log('Amazon.js > Thumbnail error'), e; }            
        
                    try {
                        if (titleSearch) {
                            scrapingResult[key].title = titleSearch.attribs?.['aria-label']?.trim() || $(titleSearch).find('span').text().trim();
                        }
                    } catch (e) { console.log('Amazon.js > Title error', e); }

                    try {
                        if (priceSearch) {
                            // Try to get full price text from Amazon's hidden accessibility span
                            let priceText = $(priceSearch).find('.a-offscreen').first().text().trim();

                            // Fallback: reconstruct from the visible parts if .a-offscreen is missing
                            if (!priceText) {
                                const symbol = $(priceSearch).find('.a-price-symbol').text().trim();
                                const whole = $(priceSearch).find('.a-price-whole').text().trim();
                                const fraction = $(priceSearch).find('.a-price-fraction').text().trim();
                                priceText = symbol + whole + '.' + fraction;
                            }

                            // Normalize to standard format (1234.99)
                            const normalizedPrice = normalizeAmazonPrice(priceText);

                            scrapingResult[key].price.current_price = parseFloat(normalizedPrice);
                        }
                    } catch (e) {
                        console.log('Amazon.js > priceSearch error', e);
                    }

                    try {
                    if (amazonChoice) {
                        scrapingResult[key].amazonChoice = true;
                    } } catch (e) { console.log('Amazon.js > amazonChoice error', e); }

                    try {
                    if (bestSeller) {
                        scrapingResult[key].bestSeller = true;
                    } } catch (e) { console.log('Amazon.js > bestSeller error', e); }

                    try {
                    if (amazonPrime) {
                        scrapingResult[key].amazonPrime = true;
                    } 
                } catch (e) { console.log('Amazon.js > amazonPrime error', e); }
                ...
                } catch (e) { console.log('Amazon.js > discountSearch error', e); };

                try {
                    if (ratingSearch) {

                        const ratingsCountSpan = ratingSearch.find('span.a-size-base.s-underline-text');
                        scrapingResult[key].reviews.total_reviews = ratingsCountSpan ? parseInt(ratingsCountSpan.text().trim()) : -2;
                        

                        const starRatingText = 
                            ratingSearch.find('i.a-icon-star-small').first().text().trim() || 
                            ratingSearch.find('i.a-icon-star-mini').first().text().trim();

                        scrapingResult[key].reviews.rating = starRatingText ? parseFloat(starRatingText) : -2; 
                    
                    } else {

                        scrapingResult[key].reviews.total_reviews = -1;
                        scrapingResult[key].reviews.rating = -1;

                    }

                        
                } catch (e) { console.log('Amazon.js > ratingSearch error', e); }
    ...
    validateRating(item) {
        ...
        if (this.scrapeType === 'products') {
            if (item.reviews.rating < 0) { return item; } // We specifically failed to find rating info due to markup changes!
...
function normalizeAmazonPrice(raw) { // For normalizing across different locales
    if (!raw) return null;
    ...
    if (lastComma > lastDot) {
        // Format like 1.234,99 (comma is decimal)
        cleaned = cleaned.replace(/\./g, '').replace(',', '.');
    } else {
        // Format like 1,234.99 (dot is decimal)
