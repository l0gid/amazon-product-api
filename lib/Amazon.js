    ...
    grabProduct(body, p) {
        ...
        for (let key in scrapingResult) {
            try {
                ...
                try {
                    if (ratingSearch) {
                        const ratingSpan = $(ratingSearch).find('span.a-icon-alt').first();

                        if (ratingSpan && ratingSpan.text()) {
                            const ratingText = ratingSpan.text(); // e.g., "4.8 out of 5 stars"
                            const ratingValue = parseFloat(ratingText);

                            scrapingResult[key].reviews.rating = ratingValue;

                            // Try to extract total reviews
                            const reviewLabel =
                                ratingSearch.parent?.parent?.parent?.next?.attribs?.['aria-label'];

                            if (reviewLabel) {
                                const totalReviews = parseInt(reviewLabel.replace(/,/g, ''));
                                scrapingResult[key].reviews.total_reviews = totalReviews;
                                scrapingResult[key].score = parseFloat(ratingValue * totalReviews).toFixed(2);
                            }
                        } else {
                            console.warn(`Amazon.js > Rating span not found or empty for ASIN ${key}`);
                        }
                    }
